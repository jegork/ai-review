...
| `RUSTY_LLM_MAX_STEPS` | cap on multi-step tool-using trajectories per review pass. Useful for tool-happy models (e.g. Anthropic) that can otherwise terminate with `finishReason: "tool-calls"` and emit no structured output. Unset = mastra's default. | — |
